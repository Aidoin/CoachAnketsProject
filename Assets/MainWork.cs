...
public class MainWork : MonoBehaviour
{
    ...
    public Sprite[] HeroesSprites; // Иконки всех героев для списка героев
    public ClassHeroy[] ClassHeroys; // Классы героев (выставленны по айди)
    public Color[] ColorHero; // Цвета всех героев
    public string[] HeroesNames; // Имена всех героев
    ...
    private void UpdateHeader()
    {
        ...
        if (InputNames[1].text != "")
            DsplayNames[1].text = ("Discord Tag:\n " + InputNames[1].text).ToUpper();
        else
            DsplayNames[1].text = "";

        if (InputNames[2].text != "")
            DsplayNames[2].text = ("Battlenet Tag:\n " + InputNames[2].text).ToUpper();
        else
            DsplayNames[2].text = "";
    }
